...
class _MapWidgetState extends State<MapWidget> {
  ...
  Offset _platformBoxPosition = const Offset(0, 0);
  ...
  Future<void> onNewPlatformState(List<Map<String, dynamic>> msgs) async {
    for (Map<String, dynamic> msg in msgs){
      if(msg.containsKey("platform_status")){
        _onUpdatePlatformPose(msg['platform_id'].toString(), msg['platform_gps_position'], msg['platform_pose']['orientation'], msg['platform_status']);
      }
    }
  }

  Future<void> onNewPlatformTrajectories(List<Map<String, dynamic>> msgs) async {
    if(msgs.isNotEmpty){
      _trajectories.clear();
      for (Map<String, dynamic> msg in msgs){
        Marker? marker = getMarkerById(msg['platform_id'], _platformMarkers);
        if (marker != null) {
  ...
  Future<void> onNewSearchMission(Map<String, dynamic> msg) async {
    if(msg.isNotEmpty){
  ...
  Future<void> onNewWaypoints(List<Map<String, dynamic>> waypoints) async {
  ...
  void _onUpdateArea(Map<String, dynamic> area) {
    AreaType areaType = AreaType.values[area['type']];
  ...
  Color _getColorFromString(String id) {
    int rSum = 0, gSum = 0, bSum = 0;
    for (int i = 0; i < id.length; i++) {
      int code = id.codeUnitAt(i);
      // Multiply each character code with its 1-based index and a unique prime per channel.
      rSum += code * (i + 1) * 1;
      gSum += code * (i + 1) * 31;
      bSum += code * (i + 1) * 3;
    }
    
    // Map each sum to a value in the range [50, 255] to avoid very dark colors.
    int r = 10 + (rSum.abs() % 246);
    int g = 10 + (gSum.abs() % 246);
    int b = 10 + (bSum.abs() % 246);
  ...
  Offset _getMarkerScreenPosition(LatLng latlng) {
    ...
    var projectedMarker = _mapController.camera.project(latlng, zoom);
    var topLeftPoint = _mapController.camera.pixelOrigin;
    var screenPosition = Offset(
      projectedMarker.x - topLeftPoint.x,
      projectedMarker.y - topLeftPoint.y,
    );
  ...
  void _addMarker(String id, LatLng latlng, double yaw, String status) {
    if(!_platformMarkerExists(id)) {
      ...
      setState(() {
        ...
        if( id == _focusedPlatformId && _isPlatformBoxVisible ) {
          _platformBoxPosition = _getMarkerScreenPosition(latlng);
  ...
  //   });
  // }
  Marker? getMarkerById(String id, List<Marker> markerList) {
    try {
      return markerList.firstWhere((marker) => marker.key == Key(id));
    } catch (e) {
      print("Marker with id '$id' not found");
      return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    onNewPlatformState(widget.platformStates);
    onNewPlatformTrajectories(widget.platformTrajectories);
    onNewSearchMission(widget.platformSearchMission);
    onNewWaypoints(widget.platformWaypoints);
    ...
    return Scaffold(
      body: FlutterMap(
        ...
        options: MapOptions(
          ...
          onPositionChanged: (camera, hasGesture) => {widget.removeCircularMenuCallback()},
        ...
        children: [
          ...
          if(_isPlatformBoxVisible)
            Positioned(
              left: _platformBoxPosition.dx,
              top: _platformBoxPosition.dy,
              child: GestureDetector(
                onTap: () {
                ...
                child: Container(
                  ...
                  child: Column(
                    children: [
                      Text('Platform ID: $_focusedPlatformId', style: const TextStyle(fontWeight: FontWeight.bold)),
                      Text('Status: $_focusedPlatformStatus'),
                    ],
                  ),
                ),
              ),
            ...
            Positioned(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: Column(
                    children: [
                      Text('Route Id: $_routeId', style: const TextStyle(fontWeight: FontWeight.bold)),
                      Text('No.: $_waypointIndex'),
                    ],
                  ),
                ),
              ),
